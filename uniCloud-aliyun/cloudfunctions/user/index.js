...
exports.main = async (event, context) => { //event为客户端上传的参数
	...
	switch (event.action) { // 通过 code 获取用户 session
		case 'code2Session':
			...
			) console.log(res_session) const success = res_session.status === 200 && res_session.data &&
			...
			}).get() if (res_user.data && res_user.data.length === 0) { // 没有用户信息，进入注册
