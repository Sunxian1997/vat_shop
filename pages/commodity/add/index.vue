		...
		<uni-card title="基本信息" class="form-card">
			<u--form labelPosition="left" labelWidth="80" :model="formData" :rules="rules" ref="formComponent">
				...
				<u-form-item label="价格" prop="baseInfo.price" borderBottom>
					<u--input type="digit" placeholder="请输入价格" border="surround" v-model="formData.baseInfo.price">
					</u--input>
				</u-form-item>

				<u-form-item label="保质期" borderBottom>
					<view class="quality-guarantee-period-box">
						<view class="quality-guarantee-period">
							<u-slider v-model="formData.baseInfo.qualityGuaranteePeriod" min="0" max="480"></u-slider>
						</view>
				...
				<u-form-item label="生产日期" prop="baseInfo.produceDate" borderBottom>
					<uni-datetime-picker type="date" v-model="formData.baseInfo.produceDate" />
				</u-form-item>
			</u--form>
			<view class="remak-box">
				<u--textarea count showConfirmBar border="bottom" v-model="formData.baseInfo.remark"
					placeholder="请输入商品详情说明"></u--textarea>
			</view>
			<view class="file-box">
				<uni-file-picker v-model="tempPicUrls" fileMediatype="image" mode="grid" @success="success" />
			</view>
		</uni-card>
		...
		methods: {
			trigger(e) {
				...
				if (index === 0) {
					...
					this.$refs.formComponent.validate().then(res => {
						const params = {
